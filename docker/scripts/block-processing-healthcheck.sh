...
POSTGRES_ACCESS=${POSTGRES_URL:-"postgresql://$postgres_user@$postgres_host:$postgres_port/haf_block_log?application_name=nft_tracker_health_check"}

# this health check will return healthy if:
# - nft_tracker has processed a block in the last 60 seconds
#   (as long as it was also after the container started, we don't want
#    to report healthy immediately after a restart)
# or
# - nft_tracker's head block has caught up to haf's irreversible block
#   (so we don't mark nft_tracker as unhealthy if HAF stops getting blocks)
#
...
CHECK="SET TIME ZONE 'UTC'; \
       SELECT ((now() - (SELECT last_active_at FROM hafd.contexts WHERE name = 'nfttracker_app') < interval '1 minute') \
               AND (SELECT last_active_at FROM hafd.contexts WHERE name = 'nfttracker_app') > '${STARTUP_TIME}'::timestamp) OR \
              hive.is_app_in_sync('nfttracker_app');"
