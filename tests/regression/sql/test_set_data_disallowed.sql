...
CALL insert_nft_set_data_op(block_num=>4, pos=>1, auth=>'alice', symbol=>'alice/XYZ', ids=>ARRAY[1], data=>'{"by": "alice"}'::jsonb);
CALL insert_nft_set_data_op(block_num=>4, pos=>2, auth=>'bob', symbol=>'alice/XYZ', ids=>ARRAY[1], data=>'{"by": "bob"}'::jsonb);
CALL insert_nft_set_data_op(block_num=>4, pos=>3, auth=>'charlie', symbol=>'alice/XYZ', ids=>ARRAY[1], data=>'{"by": "charlie"}'::jsonb);
