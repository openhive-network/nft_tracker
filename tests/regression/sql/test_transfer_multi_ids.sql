...
CALL insert_nft_transfer_op(block_num=>3, pos=>1, auth=>'alice', symbol=>'alice/X', ids=>ARRAY[1, 2], to_account=>'bob');
CALL insert_nft_transfer_op(block_num=>3, pos=>2, auth=>'alice', symbol=>'alice/Z', ids=>ARRAY[3, 0], to_account=>'dan');
CALL insert_nft_transfer_op(block_num=>3, pos=>3, auth=>'alice', symbol=>'alice/Y', ids=>ARRAY[4, 5], to_account=>'charlie');
