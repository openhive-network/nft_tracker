...
CALL insert_nft_soulbind_op(block_num=>3, auth=>'alice', symbol=>'alice/XYZ', id=>1, soulbound=>TRUE);
CALL insert_nft_transfer_op(block_num=>4, pos=>1, auth=>'alice', symbol=>'alice/XYZ', id=>1, to_account=>'null');
CALL insert_nft_transfer_op(block_num=>4, pos=>2, auth=>'alice', symbol=>'alice/XYZ', id=>2, to_account=>'null');
CALL insert_nft_transfer_op(block_num=>4, pos=>3, auth=>'alice', symbol=>'alice/XYZ', id=>1, to_account=>'alice');
CALL insert_nft_transfer_op(block_num=>4, pos=>4, auth=>'alice', symbol=>'alice/XYZ', id=>2, to_account=>'alice');
