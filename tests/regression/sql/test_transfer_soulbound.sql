...
CALL insert_nft_soulbind_op(block_num=>3, pos=>0, auth=>'alice', symbol=>'alice/AAA', ids=>ARRAY[1], soulbound=>TRUE);
CALL insert_nft_transfer_op(block_num=>4, pos=>0, auth=>'alice', symbol=>'alice/AAA', ids=>ARRAY[1], to_account=>'dan');
...
CALL insert_nft_transfer_op(block_num=>3, pos=>1, auth=>'bob', symbol=>'bob/BBB', ids=>ARRAY[2], to_account=>'dan');
